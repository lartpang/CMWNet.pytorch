...
from utils import cus_sample, load_params_for_new_conv
...
def Cus_V16BN_tv():
    ...
    load_params_for_new_conv(
        conv_layer=rgb_head[0], new_conv_layer=depth_head[0], in_dim=1
    )
...
class CMW_LM(nn.Module):
    def __init__(self, in_dims, out_dims, mid_dim):
        ...
        self.dw_l = DW(
            in_dim=max(in_dims), mid_dim=mid_dim, out_dim=min(out_dims), mode="up"
        )
        self.dw_h = DW(
            in_dim=min(in_dims), mid_dim=mid_dim, out_dim=max(out_dims), mode="down"
        )
...
class CMWNet_V16(nn.Module):
    ...
    def forward(self, data: dict):
        if self.training:
            results = self.train_forward(data)
        else:
            results = self.test_forward(data)
        return results
    ...
    def cal_loss(self, all_preds: dict, gts: torch.Tensor):
        ...
        for name, preds in all_preds.items():
            sod_loss = binary_cross_entropy_with_logits(
                input=preds,
                target=cus_sample(gts, mode="size", factors=preds.shape[2:]),
                reduction="mean",
            )
